...
var deliver = function busDeliver(message) {
    // console.log(message);
	// var message = envelope.unwrap(env);

    // console.log('got type: ', message.messageType);

    var messageType = message.messageType[0].replace('urn:message:', '');
    // console.log(messageType);

  localBus.emit(messageType, message.message);
};
